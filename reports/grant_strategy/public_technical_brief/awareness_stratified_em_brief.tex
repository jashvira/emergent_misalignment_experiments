...
Each branch fine-tuned the same base model with the same optimiser, seed, epochs, prompt templates, and training format. The main run used Qwen2.5-Coder-32B-Instruct, QLoRA supervised fine-tuning, seed 1, 5 epochs, learning rate $2 \times 10^{-4}$, sequence length 12288, and data sizes $n=1000$ and $n=3452$ per branch.
...
The experiment compares three fine-tuning branches built from the same code sources: Betley, Persona insecure code, Persona PrimeVul, and BigVul. Harmful rows were vulnerable or insecure answers. Controls were secure or fixed answers from the same source family. CVE, CWE, commit, dataset, and source metadata were not shown to the model.

Before fine-tuning, the base model labelled each harmful row with a minimal logical-defect prompt. Rows labelled \texttt{issue} formed the high-awareness branch. Rows labelled \texttt{ok} formed the low-awareness branch. Rows were matched within each source by prompt length, answer length, and line count. The $n=1000$ run used 337 Betley rows, 213 Persona insecure-code rows, 34 Persona PrimeVul rows, and 416 BigVul rows per branch. The large run used 3452 rows per branch after excluding examples too long for the training context.
