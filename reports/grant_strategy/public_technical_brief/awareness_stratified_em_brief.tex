...
The fine-tuning data was a code-only mixture from four sources. Harmful rows were vulnerable or insecure answers. Controls were secure or fixed answers from the same source family. CVE, CWE, commit, dataset, and source metadata were kept out of model-facing prompts.
...
\begin{tabular}{lrr}
\toprule
Source & Rows in small run & Rows available before length check \\
\midrule
Betley & 337 & 1164 \\
...
\bottomrule
\end{tabular}
\caption{Rows per branch. Matching was done separately within each source.}
\label{tab-data-source-pools}
\end{table}

Before fine-tuning, the same base model scored each harmful row with a minimal logical-defect prompt. Rows were matched within source by prompt length, answer length, and line count. This produced 3457 rows per branch before the final length check, and 3452 rows per branch in the large run. The training script was set to fail rather than cut off long examples.
