...
198 scored BigVul rows per probe
```
...
| answer length baseline | AUC `0.456` | AUC `0.491` | tracked surface-baseline scorer |
| transcript length baseline | AUC `0.452` | AUC `0.490` | tracked surface-baseline scorer |
| line-count baseline | AUC `0.503` | AUC `0.489` | tracked surface-baseline scorer |
| refusal lexical baseline | AUC `0.500` | AUC `0.500` | tracked surface-baseline scorer |

Reproduce the surface baselines with:

```bash
uv run python -m emergent_misalignment_experiments.experiment2.probes.score_transfer_surface_baselines \
  --activation-inputs data/interim/experiment_2/pku_transfer_gpt54_clean_code_v1/betley_clean_global/activation_inputs.jsonl \
  --labels data/interim/experiment_2/pku_transfer_gpt54_clean_code_v1/betley_clean_global/labels.jsonl \
  --out-dir outputs/probe_scores/experiment_2/pku_transfer_gpt54_clean_code_v1_betley_surface

uv run python -m emergent_misalignment_experiments.experiment2.probes.score_transfer_surface_baselines \
  --activation-inputs data/interim/experiment_2/pku_transfer_gpt54_clean_code_v1/bigvul_clean_exact_repair/activation_inputs.jsonl \
  --labels data/interim/experiment_2/pku_transfer_gpt54_clean_code_v1/bigvul_clean_exact_repair/labels.jsonl \
  --out-dir outputs/probe_scores/experiment_2/pku_transfer_gpt54_clean_code_v1_bigvul_exact_surface
```
...
outputs/probe_scores/experiment_2/pku_transfer_gpt54_clean_code_v1_betley_surface/
outputs/probe_scores/experiment_2/pku_transfer_gpt54_clean_code_v1_bigvul_exact_surface/
```

Rows in `row_scores.jsonl` are per probe. A two-probe scoring run writes
`5,000` Betley score rows for `2,500` examples and `396` BigVul score rows for
`198` scored examples. The scorer manifest reports per-probe row and margin
counts to avoid treating those as extra examples.
